...
func _ready() -> void:
	...
	_build_media_bay()
	_update_label()
	# The one front panel in the catalog that is a machine of its own. Its lamps
	# find this unit's console through host_changed, so nothing here wires them.
	if expansion_id == "satellaview":
		SatellaviewPanel.attach(self)
